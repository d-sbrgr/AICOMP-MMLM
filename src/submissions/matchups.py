...
from ..utils import Gender, get_data_directory
...
def _load_teams_data(year: int, gender: Gender) -> pd.DataFrame:
    ...
    return pd.DataFrame(
        {Columns.TEAM_ID: sorted(pd.unique(df_teams[[Columns.WTEAM_ID, Columns.LTEAM_ID]].values.ravel()))}
    )
...
def generate_matchups(year: int) -> pd.DataFrame:
    ...
    df_matchups[Columns.ID] = df_matchups.apply(
        lambda row: f"{year}_{row[Columns.LOWER_TEAM]}_{row[Columns.HIGHER_TEAM]}", axis=1
    )
    return df_matchups

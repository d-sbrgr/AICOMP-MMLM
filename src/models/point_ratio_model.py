...
class PointRatioModel(Model):
    ...
    The scores of each game can be discounted based on how long ago the game was played, and weighted based on whether it was a tournament or regular season game.
    """
    ...
    def fit(self, season: int) -> None:
        ...
        rename_rules = {Columns.WTEAM_ID: Columns.TEAM_ID, Columns.WSCORE: Columns.SCORE,
                        Columns.LTEAM_ID: Columns.TEAM_ID, Columns.LSCORE: Columns.SCORE}

        tourney_results_winning = tourney_results[[Columns.WTEAM_ID, Columns.WSCORE, Columns.SEASON]].rename(columns=rename_rules)
        tourney_results_losing = tourney_results[[Columns.LTEAM_ID, Columns.LSCORE, Columns.SEASON]].rename(columns=rename_rules)
        tourney_results_all = pd.concat([tourney_results_winning, tourney_results_losing])

        regular_results_winning = regular_results[[Columns.WTEAM_ID, Columns.WSCORE, Columns.SEASON]].rename(columns=rename_rules)
        regular_results_losing = regular_results[[Columns.LTEAM_ID, Columns.LSCORE, Columns.SEASON]].rename(columns=rename_rules)
        ...
        all_results[Columns.SCORE] = all_results[Columns.SCORE] * self._discount_factor ** (season - all_results[Columns.SEASON])
    ...
    def predict(self, matchups: pd.DataFrame) -> Iterable[float]:
        updated_matchups = matchups.merge(self._discounted_points_mean, how="left", left_on=Columns.LOWER_TEAM,
                                          right_on=Columns.TEAM_ID).rename(columns={Columns.SCORE: SCORE_LOWER}).drop(columns=[Columns.TEAM_ID])
        updated_matchups = updated_matchups.merge(self._discounted_points_mean, how="left", left_on=Columns.HIGHER_TEAM,
                                                  right_on=Columns.TEAM_ID).rename(columns={Columns.SCORE: SCORE_HIGHER}).drop(columns=[Columns.TEAM_ID])

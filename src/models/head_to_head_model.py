...
class HeadToHeadModel(WinRatioModel):
    ...
    def predict(self, matchups: pd.DataFrame) -> Iterable[float]:
        updated_matchups = matchups.merge(self._head_to_head_win_rates, how="left", left_on=[Columns.LOWER_TEAM, Columns.HIGHER_TEAM],
                                          right_on=[Columns.TEAM_ID, OPPONENT]).drop(columns=[Columns.TEAM_ID, OPPONENT])
    ...
    def _calculate_head_to_head_win_rate(self, games: pd.DataFrame) -> pd.DataFrame:
        ...
        head_to_head_games = game_wins.merge(game_losses, left_on=[Columns.WTEAM_ID, Columns.LTEAM_ID], right_on=[
            Columns.LTEAM_ID, Columns.WTEAM_ID], how="outer", suffixes=(WINNING, LOSING))

        head_to_head_games[WINNING_WTEAMID] = head_to_head_games[WINNING_WTEAMID].fillna(head_to_head_games[LOSING_LTEAMID])
        head_to_head_games[WINNING_LTEAMID] = head_to_head_games[WINNING_LTEAMID].fillna(head_to_head_games[LOSING_WTEAMID])
        head_to_head_games[LOSING_WTEAMID] = head_to_head_games[LOSING_WTEAMID].fillna(head_to_head_games[WINNING_LTEAMID])
        head_to_head_games[LOSING_LTEAMID] = head_to_head_games[LOSING_LTEAMID].fillna(head_to_head_games[WINNING_WTEAMID])
        ...
        head_to_head_games = head_to_head_games.rename(columns={WINNING_WTEAMID: Columns.TEAM_ID, WINNING_LTEAMID: OPPONENT})
        ...
        head_to_head_games[H2H_WIN_RATE] = head_to_head_games[H2H_WIN_RATE].where(
            head_to_head_games[COUNT] >= self.min_head_to_head_games,
            other=overall_ratio
        )

        head_to_head_games = head_to_head_games.drop(columns=[f"{self.WIN_RATE}", f"{self.WIN_RATE}_Opponent"])

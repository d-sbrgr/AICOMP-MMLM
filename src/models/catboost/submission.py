...
from .model import CatBoostModel, EnsemblCatBoostModel
...
def create_submission(
    ...
    project: str = "catboost",
):
    """
    Create a submission file using a trained CatBoost model from a W&B run.

    Args:
        season: Season for which to create the submission (e.g., 2025)
        run_id: W&B run ID (e.g., "qvue3sg2")
    ...
    model_cls = EnsemblCatBoostModel if is_ensemble else CatBoostModel
    ...
    submission_path = create_submission_base(
        season=season,
        model=model,
        filename=f"submission_catboost{'_' + submission_affix if submission_affix else ''}_{season}.csv",
        fit=False,
    )

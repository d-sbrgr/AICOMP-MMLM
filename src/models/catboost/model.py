...
from .config import CatBoostHyperparamConfig


class CatBoostModel(SupervisedModel):
    """
    A wrapper class to train and validate CatBoost models.
    """

    def __init__(
        self,
        data: BaseDataloader,
        params: CatBoostHyperparamConfig,
    ...
    def fit(self, season: int, start_season: int = 2003) -> None:
        ...
        self.model = CatBoostRegressor(**self.params.as_params())
    ...
    def _do_cross_validation(self, X, y):
        ...
        for fold, (tr_idx, va_idx) in enumerate(splits):
            catboost_model = CatBoostRegressor(**self.params.as_params())
            catboost_model.fit(X.iloc[tr_idx], y.values[tr_idx])

            preds = catboost_model.predict(X.iloc[va_idx])
            preds = np.clip(preds, 0, 1)
...
class EnsemblCatBoostModel(SupervisedModel):
    """
    ...
    def fit(self, test_season: int, start_season: int = 2003) -> None:
        ...
        for valid_season, (X_train, y_train), (X_valid, y_valid) in self.data.train_data(test_season, start_season):
            ...
            self.models[valid_season] = CatBoostRegressor(**self.params.as_params())

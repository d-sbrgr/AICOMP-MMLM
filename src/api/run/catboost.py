...
from ...models.catboost import CatBoostModel, EnsemblCatBoostModel
...
def sweep_catboost_ensemble(config=None, **kwargs):
    """
    Wrapper for W&B agent run for CatBoost models.
...
def run_catboost_ensemble(run: wandb.Run):
    """
    Train and validate a CatBoostModel using the given config with wandb experiment tracking.
    ...
    Args:
        run (wandb.Run): W&B experiment tracking run
    """
    run_model(run, EnsemblCatBoostModel, CatBoostHyperparamConfig, "catboost_config", cv_config=None)

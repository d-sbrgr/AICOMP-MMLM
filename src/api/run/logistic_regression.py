...
from ...models.logistic_regression import EnsembleLogisticRegressorModel, LogisticRegressionModel
...
def sweep_logistic_regression_ensemble(config=None, **kwargs):
    """
    Wrapper for W&B agent run for Logistic Regression models.
    ...
    """
    sweep_model(
        EnsembleLogisticRegressorModel,
        LogisticRegressionHyperparamConfig,
        "logistic_regression_config",
        cv_config=None,
        config=config,
...
def run_logistic_regression_ensemble(run: wandb.Run):
    """
    Train and validate a LogisticRegressionModel using the given config with wandb experiment tracking.

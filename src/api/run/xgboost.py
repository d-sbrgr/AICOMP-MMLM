...
from ...models.xgboost import EnsembleXGBRegressorModel, XGBRegressorModel
...
def sweep_xgboost_ensemble(config=None, **kwargs):
    """
    Wrapper for W&B agent run for XGBoost models.
...
def run_xgboost_ensemble(run: wandb.Run):
    """
    Train and validate an XGBRegressorModel using the given config with wandb experiment tracking.
    ...
    Args:
        run (wandb.Run): W&B experiment tracking run
    """
    run_model(run, EnsembleXGBRegressorModel, XGBHyperparamConfig, "xgboost_config", cv_config=CrossValidationConfig())

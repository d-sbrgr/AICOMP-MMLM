...
def apply_quality_to_matchups(matchups: pd.DataFrame, team_columns: list[str], quality: pd.DataFrame) -> pd.DataFrame:
    ...
    Args:
        ...
        quality: DataFrame with team quality ratings. Must include Columns.TEAM_ID and Columns.QUALITY, and optionally Columns.SEASON.
    Returns:
        DataFrame with original matchups plus:
        - {team_columns[0]}{Columns.QUALITY}: Quality rating for team in team_columns[0]
        - {team_columns[1]}{Columns.QUALITY}: Quality rating for team in team_columns[1]
        - {Columns.QUALITY_DIFF}: Difference in quality ratings (team_columns[0] - team_columns[1])
    """
    ...
    for team_col in team_columns:
        matchups_with_quality = matchups_with_quality.merge(
            quality,
            how="left",
        ...
        ).rename(columns={Columns.QUALITY: _team_quality_name(team_col)}).drop(columns=[Columns.TEAM_ID])

    matchups_with_quality[Columns.QUALITY_DIFF] = matchups_with_quality[team_a_quality_col] - matchups_with_quality[team_b_quality_col]
...
def compute_quality(
    ...
    try:
        ...
        print(f"Warning: GLM fit failed")
        return empty_result
    ...
    quality[Columns.TEAM_ID] = quality[Columns.TEAM_ID].apply(lambda x: x.split("[T.")[1].rstrip("]") if "[T." in x else x[10:14]).astype(int)

    return quality


def compute_team_quality_per_season(gender: Gender = Gender.BOTH, min_season: int = 0, max_season: int = 9999) -> pd.DataFrame:
    f"""
...
def _get_quality_base_data(gender: Gender) -> pd.DataFrame:
    ...
    regular_match_data = regular_match_data[[Columns.SEASON, Columns.LTEAM_ID,
                                             Columns.LSCORE, Columns.WTEAM_ID, Columns.WSCORE, Columns.NUM_OT]].copy()

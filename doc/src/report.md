...
This project systematically investigated the prediction of NCAA Division I basketball tournament outcomes through a comparison of statistical baselines, classical machine learning models, and deep neural networks across multiple data preparation strategies. Our best performing model, XGBoost trained on Sliding Window Averages, achieved a Brier score of 0.1168 (rank 238 out of 1,727), showing that respectable results can be achieved, even without betting market information or manual adjustment of predictions.

However, looking into the other results shows, that the performance only varies marginally for different model types. As an example, the next best models with a Brier score of 0.1171 (rank 245 out of 1'727) were CatBoost with the Weighted Seasons Averages, the neural network with the Sliding Window Averages, and a random forest with the Season Averages. Because of this, we think that the model capacities are not the bottleneck for our performance. We see bigger potential in improving on our data preparation strategies, since we were able to improve performance from 0.1178 to 0.1168 from the Season Averages approach to the Sliding Window Averages approach.

A central finding of our work is that tournament basketball exhibits inherent unpredictability that fundamentally limits predictive accuracy. The observed upset rates of 27.6% for men and 21.1% for women align remarkably well with our models' performance ceiling, suggesting that the stochastic nature of single-elimination tournaments creates a natural boundary beyond which statistical prediction cannot penetrate without incorporating additional information sources. This ceiling manifested across all our approaches as consistent plateaus at validation scores around 0.15 and test scores around 0.12, regardless of model sophistication.
...
Perhaps the most significant insight from our experiments is that performance depends more on data preparation methodology than on model complexity. Simple models like logistic regression achieved competitive results (0.1189 Brier score) that matched or exceeded far more complex models, supporting @lopez2015's conclusion about the effectiveness of modest statistical methods with informative data. The Sliding Window Averages data preparation approach improved XGBoost performance from 0.1234 to 0.1168, while neural network architecture variations showed minimal impact.
